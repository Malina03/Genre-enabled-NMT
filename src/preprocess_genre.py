...
def save_datasets(train, dev, test, tgt_lang, tgt_col, path, name):
    ...
    train[[f'en_{tgt_col}_tokens', f'{tgt_lang}_{tgt_col}']].to_csv(path_or_buf=str(path  + "/" +  name + '.train.tag.tsv'), sep='\t', index=False, header=False, quoting=3)
    dev[[f'en_{tgt_col}_tokens', f'{tgt_lang}_{tgt_col}']].to_csv(path_or_buf=str(path  + "/" +  name + '.dev.tag.tsv'), sep='\t', index=False, header=False, quoting=3)
    test[[f'en_{tgt_col}_tokens', f'{tgt_lang}_{tgt_col}']].to_csv(path_or_buf=str(path  + "/" +  name + '.test.tag.tsv'), sep='\t', index=False, header=False, quoting=3)
...
def main():
    ...
    if args.preprocess or not Path(data_folder/f'Macocu-{args.lang_code}-en-doc-format.csv').exists():    
        ...
        preprocess(data_folder, args.lang_code, 1, drop_par_duplicates = True, drop_doc_duplicates = False, keep_columns=True, info = True)
        print("Preprocessing done.")
        
    data= pd.read_csv(data_folder/f"Macocu-{args.lang_code}-en-doc-format-duplicates.csv", sep="\t", header=0)
    print("Data > 25 words: ", data[data['en_length'] > 25].shape[0])
